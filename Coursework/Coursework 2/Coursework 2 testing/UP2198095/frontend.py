...
def setUpHome():
    home = SmartHome()
    count = 0
    while count < 5:    # Loops for 5 because 5 initial devices are required to be populated
            ...
            while True:  # Validation for the consumption input.
                ...
                if consumptionRate.isnumeric():  # Checks to see if there are only numbers
                    ...
                    if 0 <= consumptionRate <= 150:  # checks to see if in range
                        ...
                        print(f'Smart Plug with consumption rate {consumptionRate} added')
                        count += 1
                        break  # Appends and break validation loop after increasing count.
                    else:
    ...
    return home     # Returns a SmartHome object with 5 valid devices populated inside
...
class SmartHomeSystem:
    ...
    def createWidgetRow(self, deviceNo, deviceType):
        ...
        self.labelVars.append(labelText)  # appended here not below so the label doesn't error 'index out of range'
        ...
        # Draw each widget inside the widget row dynamically at correct row and column
        for i, widget in enumerate(widgetRow):
            widget.grid(row=self.totalRows, column=i, padx=5, pady=5)

        # Update self.totalRows to update which row widgets are drawn in next
        self.totalRows += 1

        # Append the widget row to self.widgetRows list to keep track of widgets easily and allow for label text update
    ...
    def addGUI(self):
        ...
        addBtn = Button(addWindow, text='Add Device',
                        command=lambda: [self.addCommand(plugBoxValue.get(), doorbellBoxValue.get(), scaleValue.get()),
                                         addWindow.destroy()])  # passing in the 2 checkbox and scale values
    ...
    def addCommand(self, plugChecked, bellChecked, scale):
        ...
        # boxes are selected) a widget row is made regardless which then causes identical last device widgets
    ...
    def editGUI(self, index):
        ...
        # Checks device type and dynamically draws window
