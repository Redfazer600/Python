...
def setUpHome():
    ...
    incorrect = 0
    while count < 5:
        choice = input('\nWould you like to add a (p)lug or a (d)oorbell?\t')
        if choice.lower() == 'plug' or choice.lower() == 'p':
        ...
        elif choice.lower() == 'doorbell' or choice.lower() == 'd':
            ...
            if incorrect >= 2:
                ...
                print('\nHey! The user has given an invalid input in lego city!\n'
                      'Please enter either p/plug or d/doorbell for adding a SmartPlug or SmartDoorBell\n'
...
class SmartHomeSystem:
    def __init__(self, smartHome):
        ...
        self.scaleValue = IntVar()  # This is for the custom value slide
    ...
    def createWidgets(self):
        ...
        self.topWidgets.append(self.timeLabel)  # The clock label is global to the class but appended and drawn here to work with theming
    ...
    def createWidgetRow(self, deviceNo, deviceType):
        ...
        # Code for the custom option editing to make and append correct widget to list in correct order for loop draw
        if isinstance(deviceType, SmartPlug):
            scaleVal = IntVar()
            self.intVars.append(scaleVal)
            plugSlide = Scale(self.bottomFrame, from_=0, to=150, orient=HORIZONTAL, variable=scaleVal,
                              command=lambda value=self.intVars[len(self.intVars) - 1].get(): [self.smartHome.devices[deviceNo].setConsumptionRate(value), self.updateLabels()])
            widgetRow.append(plugSlide)
        elif isinstance(deviceType, SmartDoorBell):
            sleepToggle = Button(self.bottomFrame, text='Toggle Sleep Mode', image=self.sleepOnIcon, compound='bottom',
                                 command=lambda: [self.smartHome.devices[deviceNo].toggleSleepMode(), self.updateLabels()])
            widgetRow.append(sleepToggle)
    ...
    def deleteCommand(self, index):
        ...
        # Remove the associated option label if its a smartPlug
        # if isinstance(self.smartHome.devices[index], SmartPlug):
        #     del self.intVars[index]
        ...
        for widgetRow in self.widgetRows:
            ...
            # Check the device and config the widget accordingly
            if isinstance(self.smartHome.devices[count], SmartPlug):
                optionWidget.config(command=lambda value=self.intVars[plugCount].get(), deviceNo=count: [self.smartHome.devices[deviceNo].setConsumptionRate(value), self.updateLabels()])
                plugCount += 1
            elif isinstance(self.smartHome.devices[count], SmartDoorBell):
                optionWidget.config(command=lambda deviceNo=count: [self.smartHome.devices[deviceNo].toggleSleepMode(), self.updateLabels()])

            count += 1

        # Closes all running top levels to ensure that you cant open a window, remove the associated device and error
        for window in self.win.winfo_children():
...
############### Below are the methods specific to the challenges ###############

    # Method to update the label that contains the clock.
    def updateClock(self):
        self.clockTime += timedelta(hours=1)        # Adds 1h
        self.timeLabel.config(text=self.clockTime.strftime("%H:%M"))        # Updates the label
        self.win.after(3000, self.updateClock)      # Runs the command again after 3k milliseconds (3 seconds)
    ...
    def scheduleGUI(self):
        ...
        # Making a list of all widgets
        widgetList = [scheduleDeviceBtn, deviceNumber, deviceNumberLabel, schedulePropertyMenu, propertyLabel,
                      scheduleWhenBox, whenLabel, deScheduleDeviceBtn, confirmBtn]
        ...
        # Logic for checking the accessibility theme - I really should just make a method the amount of times it's repeated :/
        if self.darkMode:
            schedulingWindow.configure(bg="#333333")
            for widget in widgetList:
                widget.configure(foreground="#FFFFFF", background="#555555")
        elif self.customTheme:
            schedulingWindow.configure(bg=self.customBgCol)
            for widget in widgetList:
                widget.configure(foreground=self.customFgCol, background=self.customBgCol)

    # Making an accessibility GUI method
    def accessGUI(self):
    ...
    # Making a customization GUI method
    ...
    def setWindowDarkTheme(self):
        # Toggle theme variables - These are used for the themes of the toplevel windows
        self.darkMode = True
    ...
    def setWindowLightTheme(self):
        # Toggle theme variables - Again, not used in method but for the toplevel windows
        self.darkMode = False
        self.lightMode = True
        self.customTheme = False
    ...
    def setWindowCustomTheme(self):
        ...
        # Toggle theme variables - For the last time, they do the toplevel windows only :D
        self.darkMode = False
        self.lightMode = False
    ...
    def pickFgColour(self):
        ...
        colour = colour[1]  # the return of the colour chooser is a tuple with [1] being a hex code as a string value
        self.customFgCol = colour

    # Method to pick the custom background colour
    def pickBgColour(self):
        colour = colorchooser.askcolor(title="Select Color")
        colour = colour[1]  # It's still returning a tuple with a hex code string at [1].
        self.customBgCol = colour

    # Making the run method for main code body.     Modified to also run the clock update method which starts the clock
    def run(self):
        self.createWidgets()
        self.updateClock()
        self.win.mainloop()
...
# Just for fun, I made the lego error message dynamically update after 2 incorrect attempts :)
# Hi Assessor(Nad). Hope you liked the error messages, it's how I put my own spin on my coding :)
# On a serious note, only reason I didn't finish all the challenges is due to a lack of time, I struggled before being
# re-writing my code the smart way with a create widget row function.
...
#           also used spinbox and menu box in schedule window
...
# 4 marks - Device Scheduler - Created functioning clock to spec, schedule window GUI however have no schedule functionality
