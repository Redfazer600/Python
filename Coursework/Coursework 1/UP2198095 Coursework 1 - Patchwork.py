...
def main():
    ...
    for y in range(0, winSize, stepSize):
        for x in range(0, winSize, stepSize):
...
########## CHALLENGE FEATURE ##########
# The challenge feature is not completed however below is my attempt.
    ...
    def select():
        rec = Rectangle(Point(round100(click.getX()), round100(click.getY())),
                        Point(round100(click.getX() + 100), round100(click.getY() + 100)))
        rec.setWidth(6)
        rec.draw(win)

    selectMode = True
    selected = [Point(1000, 1000)]
    ...
    while True:
        ...
        closeText = Text(Point(winSize - 15, 15), 'CLOSE')
        closeText.setSize(7)
        closeText.setFill('white')
        ...
        while selectMode:
            ...
            if click.getX() in range(0, 30) and click.getY() in range(0, 30):
                ...
                print('select mode now false')
                break
            elif click.getX() in range(winSize - 30, winSize) and click.getY() in range(0, 30):
                win.close()
            else:
                ...
                for iteration in selected:
                    if round100(click.getX()) == iteration.getX() and round100(click.getY()) == iteration.getY():
                        print('selected previously')
                        print(selected)
                    else:
                        selected.append(Point(round100(click.getX()), round100(click.getY())))
                        print(selected)

        while not selectMode:
            keyPress = win.getKey()
            if keyPress == 's':
                print('You have re entered select mode')
                selectMode = not selectMode
                break
